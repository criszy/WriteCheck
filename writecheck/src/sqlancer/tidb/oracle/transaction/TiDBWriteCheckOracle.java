...
import sqlancer.tidb.transaction.TiDBTxStatement.TiDBStatementType;
import sqlancer.tidb.transaction.TiDBTxTestExecutor;
import sqlancer.tidb.transaction.TiDBTxTestGenerator;

public class TiDBWriteCheckOracle extends TxBase<TiDBGlobalState> {

    public TiDBWriteCheckOracle(TiDBGlobalState state) {
        super(state);
    }
    ...
    public List<TxStatement> genOracleSchedule(TxTestExecutionResult testResult) {
        ...
        for (TxStatementExecutionResult stmtResult : testResult.getStatementExecutionResults()) {
            ...
            if (!stmtResult.isBlocked()) {
                ...
                } else if (stmtResult.getStatement().getType() == TiDBStatementType.COMMIT
                        || stmtResult.getStatement().getType() == TiDBStatementType.ROLLBACK) {
    ...
    public List<TxStatement> genOracleWithoutCommitAndRollbackSchedule(TxTestExecutionResult testResult) {
        ...
        for (TxStatementExecutionResult stmtResult : testResult.getStatementExecutionResults()) {
            Transaction stmtTx = stmtResult.getStatement().getTransaction();
            if (!stmtResult.isBlocked() && !stmtResult.reportDeadlock() && stmtResult.getStatement().getType() == TiDBStatementType.COMMIT) {
