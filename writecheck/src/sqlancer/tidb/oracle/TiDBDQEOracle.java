...
public class TiDBDQEOracle extends DQEBase<TiDBGlobalState> {
    ...
    public void check() throws SQLException {
        TiDBSchema.TiDBTables tables = new TiDBSchema.TiDBTables(Randomly.nonEmptySubset(schema.getDatabaseTablesWithoutViews()));
        String tableName = tables.getTables().stream().map(AbstractTable::getName).collect(Collectors.joining(","));
        TiDBExpressionGenerator expressionGenerator = new TiDBExpressionGenerator(state).setColumns(tables.getColumns());
        ...
        if (operateOnSingleTable) {
            if (Randomly.getBooleanWithSmallProbability()) {
                ...
                for (TiDBSchema.TiDBColumn column : Randomly.nonEmptySubset(tables.getColumns())) {
