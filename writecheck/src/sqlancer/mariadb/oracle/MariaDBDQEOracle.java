...
public class MariaDBDQEOracle extends DQEBase<MariaDBGlobalState> {

    private final MariaDBSchema schema;

    public MariaDBDQEOracle(MariaDBGlobalState state) {
        super(state);
        schema = state.getSchema();

        MariaDBErrors.addExpressionErrors(selectExpectedErrors);

        MariaDBErrors.addExpressionErrors(updateExpectedErrors);
        ...
        MariaDBErrors.addExpressionErrors(deleteExpectedErrors);
    ...
    public void check() throws SQLException {
        ...
        MariaDBTables tables = schema.getRandomTableNonEmptyTables();
        ...
        MariaDBExpressionGenerator expressionGenerator = new MariaDBExpressionGenerator(new Randomly()).setColumns(tables.getColumns());
        MariaDBExpression whereClause = expressionGenerator.getRandomExpression();

        // MariaDBVisitor is not deterministic, we should keep it only once.
        // Especially, in MariaDBUnaryPostfixOperation and MariaDBUnaryPrefixOperation
        String whereClauseStr = MariaDBVisitor.asString(whereClause);

        // Generate a SELECT statement
        List<String> selectColumns = new ArrayList<>();
        for (MariaDBTable table : tables.getTables()) {
        ...
        if (operateOnSingleTable) {
            if (Randomly.getBooleanWithSmallProbability()) {
                ...
                for (MariaDBColumn column : Randomly.nonEmptySubset(tables.getColumns())) {
        ...
        String selectStmt = String.format("SELECT %s FROM %s WHERE %s",
                Strings.join(",", selectColumns), tableName, whereClauseStr);
        if (generateOrderBy) {
    ...
    private SQLQueryResult executeSelect(String selectStmt, MariaDBTables tables) throws SQLException {
    ...
    private SQLQueryResult executeUpdate(String updateStmt, MariaDBTables tables) throws SQLException {
        Map<AbstractRelationalTable<?, ?, ?>, Set<String>> accessedRows = new HashMap<>();
        List<SQLQueryError> queryErrors;
        try {
    ...
    private SQLQueryResult executeDelete(String deleteStmt, MariaDBTables tables) throws SQLException {
        Map<AbstractRelationalTable<?, ?, ?>, Set<String>> accessedRows = new HashMap<>();
        List<SQLQueryError> queryErrors;
        try {
            for (MariaDBTable table : tables.getTables()) {
