...
import sqlancer.sqlite3.oracle.transaction.SQLite3WriteCheckOracle;
import sqlancer.sqlite3.oracle.transaction.SQLite3WriteCheckReproduceOracle;
...
public class SQLite3Options implements DBMSSpecificOptions<SQLite3OracleFactory> {
    ...
    public enum SQLite3OracleFactory implements OracleFactory<SQLite3GlobalState> {
        ...
        WRITE_CHECK {
            @Override
            public TestOracle<SQLite3GlobalState> create(SQLite3GlobalState globalState) throws SQLException {
                return new SQLite3WriteCheckOracle(globalState);
            }
        },
        WRITE_CHECK_REPRODUCE {
            @Override
            public TestOracle<SQLite3GlobalState> create(SQLite3GlobalState globalState) throws SQLException {
                return new SQLite3WriteCheckReproduceOracle(globalState);
            }
        };
