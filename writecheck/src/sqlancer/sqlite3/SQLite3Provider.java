...
public class SQLite3Provider extends SQLProviderAdapter<SQLite3GlobalState, SQLite3Options> {
    ...
    private enum DatabaseInitStmt implements AbstractAction<SQLite3GlobalState> {
    ...
    private static int mapDatabaseInitStmt(SQLite3GlobalState globalState, DatabaseInitStmt a) {
        Randomly r = globalState.getRandomly();
        switch (a) {
            case CREATE_INDEX:
    ...
    public void generateDatabase(SQLite3GlobalState globalState) throws Exception {
        ...
        if (globalState.useWriteCheck()) {
            ...
            for (int i = 0; i < tableNum; ) {
                ...
                boolean success = globalState.executeStatement(tableQuery);
                if (success) {
                    globalState.updateSchema();
                    StatementExecutor<SQLite3GlobalState, DatabaseInitStmt> se = new StatementExecutor<>(globalState, DatabaseInitStmt.values(),
                            SQLite3Provider::mapDatabaseInitStmt, (q) -> {
